...
**2. `one continuous take` is the `WAN 3.0` house style.** Do not carry it into a specification on this route. **`references/formats/video-spec.md` does not name it** — that card is the normal format all three routes fill, and **the phrase belongs to `WAN 3.0`, which this route is not.**
