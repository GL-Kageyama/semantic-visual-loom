...
ADDED_WHY = {
    ...
    "first_frame": "**画像の**仕様を指す。⚠️ **動画の最初のコマである**——"
                   "決定（2026-09-13、著者）「全ショット画像 → それを元に動画生成」。"
                   "だからこれは**絵コンテではなく、生成の入力そのもの**である。"
...
MODELS = {
    ...
    "CHATGPT IMAGE 2.5": {
        ...
        "注": "**画像の経路を担う。全ショットが通る。**"
              ...
              "Negative を最後の一文に溶かす（実例で実測）が、**この記録は節として保つ**——"
              "`L21` と開示系列が**節の集合**として読むからである。"
              ...
              "⚠️ **繋ぐのは引き渡し票であって、記録ではない。** 記録の側は2つを別々に保つ——"
...
#:    `spec` が動画の仕様を指し、`first_frame` が画像の仕様を指す。
#:    **10本すべてが両方を持つ。**
#:
...
#:
#: ⚠️ **`negative` が違うのも実測である。** 動画の見出しは `## Negative Prompt`、
#:    画像の見出しは `## Negative（英語）` である。`specdoc.negative_prompt()` は
#:    前者を硬く持つので、**画像仕様では `None` を返す**（実測 7/7）。
#:    接頭辞一致の `specdoc.section(text, "Negative")` なら**両方読める**。
SPEC_KINDS = {
    "video": {"field": "spec", "sections": True, "negative": "Negative Prompt"},
    "image": {"field": "first_frame", "sections": False, "negative": "Negative",
              # ⚠️ **画像の仕様が持つ4欄。** `distill-essence-engine` の様式カードが
              #    使う変数であり、**画像プロンプトはこの4つを埋めることで作られる。**
              #    節の見出しは `## 主題（英語・…の4欄）` である（接頭辞で読む）。
              "vars_section": "主題", "vars": ("SUBJECT", "ACTION", "LOCATION", "ACCENT")},
}
...
FIELD_DESTINATION = {
    ...
    # ⚠️ **画像の経路そのもの。** 行き先は `distill`（画像プロンプトを作る基盤）である。
    "first_frame": "handover:distill",
}
...
DESTINATION_WHY = {
    ...
    "first_frame": "**画像の**仕様を指す。行き先は `distill`——"
                   "画像プロンプトを作る基盤であり、**このリポジトリはそれを回さない**"
                   "（著者が手で投入する）。"
                   "⚠️ **これは絵コンテではない。** 生成された画像は**動画の最初のコマ**として"
                   "Wan へ渡る——だから画像が狙いと違えば、**その上に積む動画は10倍の無駄になる**"
