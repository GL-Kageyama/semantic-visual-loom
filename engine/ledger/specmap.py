...
ADDED_WHY = {
    ...
    "aim": "**狙い。**このショットが15の文法のどれを試すのか。"
           "**§1–20 に欄が無い**——仕様が書くのは「どう作るか」であって、"
           "**「何を試すのか」ではない。**"
           "⚠️ **ゆえに `enum` にもできない。****15の文法は、このリポジトリの中に"
           "目録を持たない**（`11. MOTION` のような節でも、`rolemap` のような表でもない）。"
           "**目録の無い符号を並べた欄は、`failure_type` の `A`/`B`/`C` と同じ形になる**"
           "——書けるが、意味を持つ文書がどこにも無い。"
           "だから自由文である。決定（2026-09-21、著者）D-13。",
}
...
MODELS = {
    "WAN 3.0": {
        ...
        "注": "**動画の経路を担う。全ショットが通る。**"
              ...
              "⚠️ **この経路は `MODEL_ROUTE` も `MODEL_UNRECEIVED_SLOTS` も空である**"
              "——**禁じる語も、受け取れない欄も無いと測った**からである。"
              "**ゆえに `MODEL_ROUTE_DOC` は自前の文書を持たず、共有のノーマルフォーマット"
              "（`references/formats/video-spec.md`）を指す。**"
              "**`WAN 3.0` 固有で言うことが無いのであって、書き忘れではない**"
              "（他は測って空にした。この1行が無いと、非対称が事故に見える）。",
    },
    "MINIMAX H3": {
        "slug": "minimax-h3",
        "種別": "video",
        "出所": "著者の決定（2026-09-20）——取り込み元は資料"
                "『GPTimage2.5×MiniMaxH3による動画化手法』（①絵コンテ画像 → ②その画像を動画にする）",
        "注": "**動画の経路を担う2つ目である。WAN 3.0 と択一である**——"
              "1つのショットは1つの `spec` を持ち、その仕様が名乗る §18 の見出しがモデルを決める"
              "（`_model_of` は**最初の §18** を読む）。"
              "⚠️ **ゆえに、いま1つのショットが両方の経路を持つことはできない**"
              "（`spec` は `string` であり、`L11` は §18 が2つ在ると順序違反として鳴らす）——"
              "**「同じショットを WAN と H3 の両方で撮って較べる」は、まだできない。** 穴である。"
              "⚠️ **入力が違う。** WAN は文から生成し `key_image` へ見た目を寄せる。"
              "**H3 は絵コンテ画像そのものを、動画全体の設計図として受け取る**——"
              "資料②「添付したストーリーボード画像を、動画全体の絵コンテとして使用してください」。"
              "**だから添付は「見せ場の1枚」（`key_image`）ではない**——"
              "あちらは1つの瞬間の見た目であり、こちらは**順序と構図の設計**である。"
              "⚠️ **単位が違う。** 著者の決定（2026-09-20）——**1生成＝全コマ。**"
              "固定方針は「ショットが単位、1ショット＝1変化」である。この経路の下では"
              "**絵コンテの12コマは、1つのショットの内側に在る**——"
              "著者の裁定（2026-09-20）により**コマはそれぞれ独立したシーンとして扱われ**、"
              "**1つの変化がその中で起きる**（取り込み第一号は、第二巻第1話の運搬が止まること）。"
              "⚠️ **絵コンテには文字が描かれる**（①は日本語の説明文とコマ番号を要求する）。"
              "**ゆえに絵コンテは画像の経路ではない**——あちらは `L21` が"
              "「画面に字幕を出すな」を床として要求する。**絵コンテは撮る前の紙である。**"
              "⚠️ **その紙を作るのは `distill-essence-engine` である**（`handover: distill`）——"
              "様式は `luminous-anime`、フォーマットは `storyboard`。"
              "⚠️ **出力は4秒から15秒である**（ベンダー公表、2026-09-20 に確認）——"
              "**16コマはこの枠に入らない。** ゆえに取り込み第一号のコマ数は**12**である。"
              "⚠️ **この経路に掛かる制約——渡す文字列と、その紙の両方——は "
              "`docs/h3-route.md` に列挙してある。** うち文法に属するものは `L28` が読む"
              "（禁じる語と理由は `MODEL_ROUTE`）。",
    },
    "SEEDANCE 2.5": {
        "slug": "seedance-2.5",
        "種別": "video",
        "出所": "著者の決定（2026-09-21）——事実の出所は BytePlus（ModelArk）公式ドキュメント。"
                "⚠️ **動機は「文字に強い」ではない。** **その主張は一次情報で裏付けられなかった**"
                "（下の註）——**著者は動機を差し替えて取り込むと裁定した**"
                "（2026-09-21）。**主目的は「`Negative Prompt` に行き先が無い経路」を"
                "基盤に名指しさせることである。**",
        "注": "**動画の経路を担う3つ目である。**"
              "⚠️ **この経路は `Negative Prompt` を受け取れない**——"
              "ゆえに `MODEL_UNRECEIVED_SLOTS` に行を持ち、**`L30` が鳴らす。**"
              "公式に否定として扱われるのは**字幕と音声だけ**である"
              "（`\"No subtitles.\"` / `\"No BGM\"` / `\"No audio.\"`）——"
              "**専用のパラメータが無く、残りはただの散文として読まれる。**"
              "**効かなかった否定は、渡さなかった否定と見分けがつかない。**"
              "⚠️ **しかし `no on-screen subtitles` の床は、この経路でむしろ強く効く**——"
              "**この経路には字幕を出す公式の記法が在る**（`【】`）からである。"
              "**字幕（不随意的な焼き込み）と作中の文字は別物である**——"
              "**区別を書かないと、次の読者は床を外して字幕を許す。**"
              "⚠️ **実在の顔を含む参照は受け取らない。** 公式の警告そのまま——"
              "`Seedance 2.5 does not support directly uploading reference images or videos "
              "containing real human faces.` **実在の人物を撮る作品は、この経路を持てない。**"
              "⚠️ **`Shot 1 / Shot 2` の複数カットが公式記法である。** "
              "**固定方針「30秒セグメントで割らない」と正面から衝突する**——"
              "**1つのショットが1つの変化を持つ**という単位を、この記法は1生成の中で割る。"
              "**この経路を使う仕様は、この衝突を承知のうえで書かれる**（`L30` は読まない）。"
              "⚠️ **一次情報で確認した事実**——尺は**4〜30秒**（`-1` ならモデルが最適尺を選ぶ。"
              "**2.0系の4〜15秒から倍になった**）／解像度は 480p・720p・**1080p（10bit）**／"
              "**24fps**／比は `16:9` `4:3` `1:1` `3:4` `9:16` `21:9` `adaptive`。"
              "**24fps と 16:9 と 1080p は `bible.video` と一致する。**"
              "参照素材は**1リクエスト最大50点**（画像30＋動画10＋音声10）で、"
              "`role` に `first_frame` / `last_frame` を持つ——"
              "**ゆえに「最初のコマ」を渡す型が在る**（`WAN 3.0` の添付とは別の渡し方である）。"
              "プロンプト内で **`0-3s:` `3-6s:` の秒単位の区間指定**ができる。"
              "音声の記法は `()` 音楽／`<>` 効果音／`{}` 台詞／`【】` 字幕。"
              "⚠️ **確認できなかったこと**——**「文字に強い」という主張そのものが一次情報に無い。**"
              "ByteDance の公式ブログにも BytePlus の公式ドキュメントにも文字レンダリングの主張は無く、"
              "公式が言うのは**逆**である（`minimizes uncontrolled occurrences in subtitles and "
              "background music`）。**`Text generation` の節は 2.0系の名義**であり、"
              "**2.5 の差分リストに文字の項目は無い。** 文字の精度の実測値もベンチマークも"
              "**公式には存在しない**——第三者の数値（Text Fidelity 2.5: 93.3）は"
              "**ページ上で「評価本数 0」**と表示され取得のたびに揺れた。**根拠に使えない。**"
              "⚠️ **火山引擎（中国）の公式ページは JS 描画で取得できない**——"
              "**モデル ID・提供開始日・価格は第三者記事のみである。ゆえにここに書かない。**"
              "⚠️ **この経路に掛かる制約は `docs/seedance-route.md` に列挙してある。**"
              "うち文法に属するものは `L28` が読む（禁じる語と理由は `MODEL_ROUTE`）——"
              "**いまこの行は空である**（実測 2026-09-21 に1本撮った）。"
              "**空は「測っていない」ではなく「語に帰せていない」である**——"
              "撮った1本は §19 と3点で食い違い（解像度・ビスの数・印刷の黒）、"
              "**そのどれも、語に帰す証拠が無い。**",
    },
    "CHATGPT IMAGE 2.5": {
...
#: **経路ごとに、その §18 が運んではならない語。** `L28` が読む。
#:
#: ⚠️ **ここは「その語が正しいか」を決めない。「どの経路の語か」を決める。**
#:    意味の判定はしていない——**その文字列が、その経路に属するかどうか**だけを見る。
#:    `MODES` の隣に `MODE_DEMANDS` が在るのと同じ理由で、**`MODELS` の隣に置く**
#:    ——**動画の経路（`種別` が `video`）について、`MODELS` と両方向に閉じる**
#:    （自己検査と `L28` がそれを見る）。⚠️ **画像の経路は §18 を持たないので、
#:    ここに行を持たない**——`L28` が読むものが無い。
#:
#: ⚠️ **語彙を持たない経路の行を落としてはいけない。** 落とせば
#:    「禁じる語が無い」と「書き忘れた」の区別が消える
#:    （`MODE_DEMANDS["motion"]` が空のタプルで在るのと同じ規律である）。
#:
#: ⚠️ **語は `(語, 理由)` の対で持つ。** 理由は検査のためではない——
#:    **次に読む者のためである。** 語だけを残せば、なぜそこに在るかが消え、
#:    **次の者が、同じ欠陥を出した語を「根拠が無い」と消してよいと読む。**
#:
#: ⚠️ **足すのは、実際に欠陥を出した語だけである。** 推測で足せば**正しい仕様の上で鳴る。**
#:    却下した語とその実測は `engine/ledger/README.md` と `HISTORY.md` に残す——
#:    **「思い出した者だけが守る」にしないためである。**
#:
#: ⚠️ **却下が4つ出た**（2026-09-20、この層を立てた日の実測）。**4つとも、直した仕様の上で鳴る:**
#:      `no additional person` … 直した3本の §18 が
#:          `no additional person beyond the storyboard` を持つ。**出典の「不必要な」を
#:          落とした形そのもの**であり、**正しい側が当たる**——ゆえに検査の語にできない。
#:      `no colleague at the counter` … 直した形は `no colleague invented at the counter`。
#:      `no cut`（裸） … 3本が `no cuts to unrelated locations` を持つ——
#:          **カードが残せと言っている句である。**
#:      `continuous`（裸） … 3本の §18 `Style Motion` が `particles fall continuously` と言う
#:          （**様式カードからの不変の引用**である）。
#:    **当たらない脆さは受け入れる。当たりすぎる脆さは受け入れない**——
#:    註も同じ文字列に当たるので、直した仕様の上に註を並べれば
#:    **雑音になり、註を読まなくさせる。**
#:
#: ⚠️ **経路で絞ることが必須である**（実測 2026-09-20）。このリポジトリの動画仕様は**18本**、
#:    うち `WAN 3.0` が**15本**、`MINIMAX H3` が**3本**。**`one continuous take` は
#:    その15本すべての §18 に在り、それは正しい**——**この経路の家風である。**
#:    絞らねば**15件の偽陽性**が出る。
#:
#: ⚠️ **平板な否定が欠陥かどうかは、文字列の形ではない。** `no additional person` が
#:    欠陥なのは、**§18 の `Master`／`Visual` が置く実体と衝突するから**である
#:    ——**そこには意味の照合と実体の台帳が要り、この層にはできない。**
#:    この層が捕まえるのは**語そのものが別の経路のものである場合**だけである。穴は
#:    `engine/ledger/README.md` の穴の表に書く。
MODEL_ROUTE = {
    # **禁じる語を持たない経路である。** 空のタプルは「書き忘れ」ではない——
    # `one continuous take` はこの経路では正しい（実測で22本が §18 に持つ）。
    # ⚠️ **走査した集合を書く**——**「ショット記録が `spec:` で指す `WAN 3.0` の仕様」22本**である
    # （2026-09-22 実測。⚠️ リポジトリ全体の `WAN 3.0` は121本であり、その数ではない）。
    # **リポジトリ全体でこの語を持つ `WAN 3.0` の仕様も、同じ22本である**——
    # ゆえに「到達する22本」と「持つ22本」は集合として一致する。
    # ⚠️ **この句は、もうカードの冒頭行ではない**（2026-09-22）。カードは
    # **3経路すべてが埋めるノーマルフォーマット**へ改められ、冒頭行は
    # `Granularity×time: whole arc × unfolding` になった——
    # **句はこの経路のものであり続けるが、カードはもう名乗らない。**
    # ゆえに下の理由の文面も「冒頭行」を引かない。
    "WAN 3.0": (),
    # ⚠️ **3語とも、直した3本の §18 には1つも無い**（実測 2026-09-20）。
    # 在るのは `habits-ch02-seg01` の §20 `Observed Problems` であり、
    # **原因として引用している**——ゆえに `L28` は §18 の外を読まない（`_section_body`）。
    # **ファイル全体を走査すれば、直した仕様の上で鳴る**（`L4` が踏んだ形である）。
    "MINIMAX H3": (
        ("one continuous take",
         "**`WAN 3.0` の家風である**（その経路はこの句を禁じていない——"
         "`MODEL_ROUTE` の `WAN 3.0` が空であることがそれである）。"
         "この経路では**コマはそれぞれ独立したシーンであり、自然なアニメーションで繋がる**——"
         "資料②「各コマを独立したシーンとして扱い、静止画と静止画の間を自然なアニメーションで"
         "つないでください」。**この句を写した仕様が、実際に欠陥を出した**"
         "（実測 2026-09-20、`habits-ch02-seg01` の §20）。"),
        ("never by a cut",
         "**無条件の `no cut` は、コマが場所を変える唯一の合法な手段を塞ぐ。**"
         "塞がれた生成器は、**二つの場所をひと続きの部屋として走らせた**"
         "（実測 2026-09-20、`habits-ch02-seg01` の §20——受付の机と車内が一つの空間になった）。"
         "**禁じてよいのは、無関係な場所へのカットだけである。**"),
        ("one and the same man",
         "**主人公を「唯一の人物」へ畳む。** この経路の §18 は二人目を置ける"
         "（実測 2026-09-20——受付の人物が、蒼と同一人物として描かれた）。"
         "正しい形は `keep the protagonist the same person`——"
         "**彼は主人公であって、唯一の人物ではない。**"),
    ),
    # ⚠️ **この行は、1本撮ったあとも空である**（実測 2026-09-21、`habits-ch02-seg07`）。
    # 基盤の規律は「**足すのは、実際に欠陥を出した語だけである。推測で足せば
    # 正しい仕様の上で鳴る**」——**撮った1本は §19 と3点で食い違い
    # （解像度・ビスの数・印刷の黒）、そのどれも語に帰せていない。**
    # **空は「書き忘れ」でも「測っていない」でもない。**
    # （`WAN 3.0` の空とは理由が違う——あちらは「禁じる語が無いと測った」である。）
    "SEEDANCE 2.5": (),
}

#: **経路ごとの、制約を列挙した文書。** `L28` が違反の文面に引く。
#:
#: ⚠️ **この表が在る理由は、硬く書いた文面が1つ目の経路で嘘になったことである。**
#:    `L28` は `docs/h3-route.md` を直書きしていた——`SEEDANCE 2.5` を登録した日に、
#:    **その文面は3つ目の経路について嘘になった**（実測 2026-09-21）。
#:    **経路の名前を検査の本文に書けば、経路が増えるたびに書き直すことになる。**
#:
#: ⚠️ **文書を持たない経路の行も要る。** 落とせば「引く先が無い」と「書き忘れた」の
#:    区別が消える（`MODEL_ROUTE` の空のタプルと同じ規律）。
#:    **値が `None` なら、`L28` の文面は文書を引かない。**
#:
...
#: **経路が受け取れないスロット。** `L30` が読む。形は `MODEL_ROUTE` と同じ `(スロット, 理由)`。
#:
#: ⚠️ **これは推測ではない。測った事実である。** ゆえに `L28` と同じ重さで鳴らせる——
#:    **「その語が別の経路のものか」ではなく「その経路が、その欄を持っているか」**である。
#:
#: ⚠️ **空のタプルは「受け取れる」の主張ではない。** **測っていないからである**——
#:    `MODEL_ROUTE` の `WAN 3.0: ()` と同じ規律で、
#:    **測った範囲と主張した範囲を同じにする。**「確かめて受け取れる」と
#:    「確かめていない」は、**空のタプルでは区別できない**——ゆえに註に書く。
#:
#: ⚠️ **宣言されたスロットに中身が在るとき、`L30` は鳴る。**
#:    中身が在ることは欠陥ではない——**しかし、その中身が届かないことは事実である。**
#:    **効かなかった否定は、渡さなかった否定と見分けがつかない。**
#:
#: ⚠️ **作品が引き受ける道を残す**（`ROUTE_LIMITS_KEY`）——**除外は作品が書く。
#:    どの作品が特別かを覚えているエンジンは、エンジンであることをやめている。**
#:
#: ⚠️ **`MODELS` の動画の経路と両方向に閉じる**（`L30` の①と自己検査がそれを見る）。
MODEL_UNRECEIVED_SLOTS = {
    # ⚠️ **測っていない。** この経路で否定が届かなかった実測は、まだ無い。
    "WAN 3.0": (),
    # ⚠️ **測っていない。** この経路は `docs/h3-route.md` の水系に在るが、
    #    **「受け取れないスロット」としてはまだ1件も出ていない。**
    "MINIMAX H3": (),
    "SEEDANCE 2.5": (
        ("Negative Prompt",
         "**この経路は、否定を専用のパラメータで受け取らない。**"
         "公式に否定として扱われるのは**字幕と音声だけ**である"
         "（`\"No subtitles.\"` / `\"No BGM\"` / `\"No audio.\"`）——"
         "**残りは、ただの散文として読まれる。**"
         "ゆえに §18 の `Negative Prompt` は、この経路では**床ではなく助言になる。**"
         "⚠️ **効かなかった否定は、渡さなかった否定と見分けがつかない**——"
         "**描かれてから分かる。**"),
    ),
}

#: **作品が、経路の制限を承知で使うと宣言する席。** `L30` が註で報告する。
#:
#: ⚠️ **`WAIVED_KEY` と同じ規律である**——**除外は作品が書く。**
#:    値は文字列の列で、`"<モデル>: <スロット>"` の形を取る（例: `"SEEDANCE 2.5: Negative Prompt"`）。
#:
#: ⚠️ **これは「違反を消す」ではない。** **違反を、著者が名指しで引き受けた註に変える。**
#:    **報告しない除外は、通った検査に見える。**
ROUTE_LIMITS_KEY = "route_limits_accepted"
...
#: **基盤が必ず付ける禁制。** 決定（2026-09-18、著者）——二つである。
#:
#:   ① **字幕は、こちらから指定しない限り、必ず不要である。**
#:   ② **BGM も、出さない。**
#:
#: ⚠️ **作品の禁制（`bible.negative_base`）とは別の層である。**
#:    あちらは**作品が決める**——「窯の中を見せない」のように、作品ごとに違う。
#:    こちらは**基盤が決める**——どの作品でも同じであり、**作品が書き忘れても消えない。**
#:    `L21` は**両方を足したもの**を要求する（覆うであって、等しいではない）。
#:
#: ⚠️ **なぜ基盤の側に置くか。** `schemas/bible.schema.json` はこの欄を
#:    「全ショットに必ず付く禁制」と説明しながら、**「作品台帳に置くのか、
#:    基盤が自動で付けるのかは未決定」**と書いていた。**未決定のあいだは、
#:    書き忘れが黙って消える。** 実測（2026-09-18）——この基盤で作った作品の動画に、
#:    **中国語の字幕が焼かれた。** その作品の Negative は `no on-screen subtitles` を
#:    持っていた。**それでも焼かれた。** 禁制は**床であって、保証ではない**——
#:    だが床が無ければ、**思い出した者だけが守ることになる。**
#:
#: ⚠️ **ここに語を足すときは、既存の全作品の Negative が既に持っている語を選ぶ。**
#:    足せば `L21` が全作品で鳴る。**鳴るのが正しい**——ただし**正しい鳴り方をするのは、
#:    その語が本当に基盤のものであるときだけである。**
#:
#: ⚠️ **2026-09-18、② を足した。**（決定——「**リポジトリとして、BGM も出さないことを
#:    明記しよう**」）⚠️ **そのとき `projects/hitosara` は、BGM の節を一字も持っていなかった**
#:    （動画10本・画像10本を走査して零件）。**ゆえに祖父条項が要った**——
#:    `bible.base_negatives_waived` である。**除外は作品が宣言し、`L21` が註で報告する**
#:    （**報告しない除外は、通った検査に見える**）。
#:
#: ⚠️ **両経路に掛かる**（決定——字幕と同じ扱いである）。**1枚は音を持てない**ので、
#:    画像の Negative に入れた `no background music` は**効かない**——が、
#:    **二経路の禁止が一字も違わないこと**を採る作品では、写しがそれを運ぶ。
BASE_NEGATIVES = (
    "no watermark",
...
#: 作品が「この基盤の禁制は、この作品には掛からない」と宣言する欄の名前。
#:
#: ⚠️ **祖父条項である。** 基盤の禁制は増える（② がそうであった）——
#:    そのとき、**規則より前に書かれた作品**には、書いていなかったことを理由に赤が立つ。
#:    **規則は遡らない。** ゆえに除外の席が要る。
#:
#: ⚠️ **除外は作品が書く。** 基盤が作品名を覚えてはならない——
#:    **基盤が特定の作品を知れば、その作品は基盤の一部になる。**
#:    ここに在るのは**欄の名前だけ**であり、誰を除くかは `bible.yaml` が持つ。
#:
#: ⚠️ **`L21` は除外を註で報告する。** **黙って除けば、赤が消えた理由が見えなくなる。**
WAIVED_KEY = "base_negatives_waived"
...
#:   `channel:<種類>`           … `text_channel` に、その `kind` の行が**1つ以上**あること
#:
#: ⚠️ **`composite` が要求するのは「`text_channel` が非空」ではない**（裁定 2026-09-21）。
#:    `composite` は「画は層の合成である」を意味し、**焼くのは `timeline` である**
#:    （`TEXT_CHANNEL_KINDS["overlay"]`）——**生成器が描く `lettering` は、焼かない。**
#:    だから `lettering` だけを持つ合成のショットは、**何も焼かないのに非空である。**
#:    ⚠️ **要求を `channel:overlay` へ締めた。** 締める前は、その形が通っていた。
#: ⚠️ **逆向きは成り立たない**——`motion` のショットも `text_channel` を持てる。
#:    **片方向だけである。**
MODE_DEMANDS = {
    "still": ("section:11.",),
    "composite": ("section:11.", "channel:overlay"),
    "motion": (),
}

#: **`text_channel` の種類 → 行き先。** `L19` が `FIELD_DESTINATION` と突き合わせて閉じる。
#:
#: ⚠️ **欄ごとではなく、種類ごとに決まる。** `FIELD_DESTINATION["text_channel"]` は
#:    **欄の行き先**（3つ）であり、**どの種類がどこへ行くかは言っていない。**
#:    `lettering` の行き先は `prompt:Master Prompt` であって `edit:timeline` ではない——
#:    **生成器が描くものを、タイムラインが焼けば二重になる。**
#:
#: ⚠️ **両方向に閉じる**（`L19`）——スキーマの `enum` と、この表の鍵と。
#:    片方だけ足せば、**足した種類はどこにも行かないか、行き先の無い種類になる。**
TEXT_CHANNEL_KINDS = {
    # 基盤が焼く。**生成器は文字を描かない**（§2.6）。
    "overlay": "edit:timeline",
    # §14 の声の行。**行き先は欄と同じである**（種類で割れない唯一のものである）。
    "voice": "prompt:Audio Prompt",
    # **生成器が作中の文字として描く**（裁定 2026-09-21、著者）。
    # ⚠️ **可読度は名乗らない。** それはその話の §18 が決める。
    "lettering": "prompt:Master Prompt",
}
...
FIELD_DESTINATION = {
    ...
    # ⚠️ **この欄は `kind` で3つに割れる**（`TEXT_CHANNEL_KINDS`）。
    ...
    #    `lettering`——**生成器が作中の文字として描く**（裁定 2026-09-21、著者）。
    #    だから `prompt:Master Prompt` へ行く——**タイムラインではない。**
    #    **行き先が3つ在るのは、欄が3種類を束ねているからである。**
    #    ⚠️ **対応表は `TEXT_CHANNEL_KINDS` が持つ。** `L19` が両方向に閉じる。
    #    ⚠️ **この割り方は `L19` ではなく引き渡しの層が行う**（段2）——`L19` が見るのは
    #    「その行き先が語彙に在り、送り先が実在するか」だけである。
    "text_channel": ("edit:timeline", "prompt:Audio Prompt", "prompt:Master Prompt"),
    ...
    # ⚠️ **どこへも渡らない。** 狙いは**読み手のための欄**である——著者と検証が
    #    「このショットは何を試したのか」を後から読む。**生成器は文法を試さない。**
    #    ⚠️ **渡らないことは、落ちることではない**（`自前` の註を見よ）。
    "aim": "自前",
}
...
DESTINATION_WHY = {
    ...
    "aim": "⚠️ **生成へは渡らない。** 狙いは**読み手のための欄**である——"
           "著者と検証が、生成のあとに「このショットは何を試したのか」を読む。"
           "**生成器は文法を試さない。**"
           "⚠️ **渡さないのは、訳せないからではない。**"
           "英語にはできる（`aim` は自由文である）——"
           "**したくないのである。**狙いをプロンプトに混ぜれば、"
           "**仕様適合（層2）と狙い（層3）が同じ文字列を読むことになる**"
           "——`L26` の規律と逆向きである。"
           "⚠️ **そして渡らないことは、落ちることではない**"
           "（`CLAUDE.md`「生成に渡す文字列を日本語にしない」——`自前` の欄は"
           "そもそも渡らない。**渡らないことと、渡すはずのものが届かないことは別である**）。",
    ...
    "text_channel": "⚠️ **この欄は `kind` で割れる**（`TEXT_CHANNEL_KINDS`）。"
                    ...
                    "`lettering`——**生成器が作中の文字として描く**（裁定 2026-09-21、著者）。"
                    "⚠️ **`lettering` は「読ませる」ではない。** 種類が主張するのは"
                    "**「その文字は生成器が描く」**という一点だけである——"
                    "**可読度は、その話の §18 `Negative Prompt` が決める。**"
                    "**作品の法を種類に焼き付けない**——焼き付ければ、"
                    "看板や店名を読ませる作品に狭すぎる。"
                    "⚠️ **字幕と作中の文字は別物である。** 字幕（不随意的な焼き込み）は"
                    "どの種類でもなく、**床 `no on-screen subtitles` が禁じる**——"
                    "そして**経路によっては字幕を出す公式の記法が在る**（`SEEDANCE 2.5` の `【】`）。"
                    "**区別を書かないと、次の読者は床を外して字幕を許す。**"
                    "**行き先が3つ在るのは、欄が3種類を束ねているからである。**"
