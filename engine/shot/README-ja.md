...
⚠️ **1つの鍵だけ例外である——`REF_STYLE`**（`L31`・`check_style_reference`）。
**読む。読んでも推測にならないからである**——上の3つの綴りを**1つの正規表現が受け**、
**2つの語彙（カード名・パス）は、比べる前に1つの名へ畳まれる**（末尾の要素を取り、`.md` を落とす）。
⚠️ **ゆえに `L31` は、どちらの語彙が正しいかを決めない**——**両方を比べられるようにするだけである。**
