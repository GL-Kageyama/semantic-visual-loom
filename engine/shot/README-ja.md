...
⚠️ **例外は2つの鍵である——`REF_STYLE`（`L31`）と `REF_FORMAT`（`L32`）。**

**`REF_STYLE` は `L31`（`check_style_reference`）が読む。読んでも推測にならないからである**
——上の3つの綴りを**1つの正規表現が受け**、**2つの語彙（カード名・パス）は、比べる前に
1つの名へ畳まれる**（末尾の要素を取り、`.md` を落とす）。⚠️ **ゆえに `L31` は、どちらの語彙が
正しいかを決めない**——**両方を比べられるようにするだけである。**そして、その結果を
**家（`bible.style`）と突き合わせる。**

**`REF_FORMAT` は `L32`（`check_format_reference`）が読む。カードそのものへ届くためである。**
`L31` が名乗りを家と比べるのに対し、**`L32` は「名指しされたカードが読めるか」「`## Negative` を
宣言しているか」を訊く**——`Negative Prompt` は
`§16 ＋ このカードの Negative ＋ 様式カードの Negative` と定められており
（`specmap.PROMPT_SLOT_SOURCE`）、**そのカードの `Negative` を読む者はいなかった。**
⚠️ **`L32` は家と比べない——`REF_FORMAT` に家は無い**（[`docs/cards-ja.md`](../../docs/cards-ja.md) を見よ）。

⚠️ **他の鍵は、いまも読まない**——上の規則は、あちらにそのまま効く。
