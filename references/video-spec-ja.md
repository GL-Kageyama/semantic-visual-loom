...
⚠️ **経路は時間の文法を共有しておらず、この文書の冒頭の行はそのうち片方しか書いていない。**
`Granularity×time: whole arc × one continuous take` は **`WAN 3.0` の家風**であり、
**`MINIMAX H3` では成り立たない。** **あちらでは、各コマがそれ自体で1つのシーンであり、
次のコマとは自然なアニメーションでつながる**——⚠️ **だから §18 は、その継ぎ目を禁じてはならない。**
**`no cut` を無条件の規則として書くと、コマが場所を変える唯一の合法な手段が塞がり**、
生成器は2つの場所を1つの部屋として続けてしまう。
**無関係な場所へのカットは禁じ、遷移そのものは禁じない**——そしてコマが場所を変えるときは、
**どちらなのかを仕様が書く。** 出典は `カット` という語を使わないので、切るか繋ぐかはショットが決める。
⚠️ **冒頭の行を `H3` の仕様へ写した読み手は、もう一方の経路の規則を写している**——
この警告を生んだ失敗は `HISTORY.md` に記録してある。
**この段落を読むのは `L28` である**（語と理由は `specmap.MODEL_ROUTE` に在る）。
