...
動画の経路は2つ登録されている——**`WAN 3.0` は `key_image` を添え**、
...
⚠️ **1つのショットは1つの `spec` を持つ**ので、**1つのショットが両方の経路を持つことはできない**——同じショットを両方で撮って較べることは、まだできない。

⚠️ **2つの経路は時間の文法を共有しておらず、この文書の冒頭の行はそのうち片方しか書いていない。**
