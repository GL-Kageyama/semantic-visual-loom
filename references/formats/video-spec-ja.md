...
- **目的**: 叙述（再体験・誘引） ／ **粒度×時間**: 弧の全体 × 並べる ／ **サイズ・比率**: シネマ 16:9、`DURATION` のクリップ
- **要約**: 出力が**時間を持つ**唯一のフォーマット。クリップの持続を弧へ不均等に配り、静止画には語彙が無い4つの軸——時間・運動・カメラの移動・音——を担う。

## このフォーマットだけが違う一点

このエンジンの他のフォーマットはすべて時間を**捨てて**静止した面に着地する。これだけは時間を**畳み込む**。圧縮の対象は「語る一点」ではなく**語る持続**であり、選択の問いもそれに応じて変わる。
...
**音の上で終える、その後ではなく**。最後の一秒は、観る者が「次があるか」を決める場所。フックに着地する——その後に解決のビートを足さない。
...
⚠️ **見出しが族である節は §18 だけである**: `18. <モデル名> PROMPT MAPPING`。
その名は登録簿（`specmap.MODELS`）と突き合わせられ、**その名が経路を選ぶ。**
動画の経路は3つ登録されている——**`WAN 3.0` は `key_image` を添え**、
**`MINIMAX H3` は絵コンテの画像を添え、画像の経路をまったく持たない**——
あちらの紙は `distill-essence-engine` が作り（フォーマット `storyboard`、様式 `luminous-anime`）、
`specs/board/<id>-board.md` に住む。
そして**`SEEDANCE 2.5` は既定では何も添えない**——長い一つのプロンプトを読み、
**そのプロンプトが自前の時計を持てる**（`0-3s:` `3-6s:`）——一方で
**`Negative Prompt` の欄は、床としては受け取られない**（`L30`）。
⚠️ **この経路の制約のすべては [`docs/seedance-route.md`](../docs/seedance-route.md) に在る。**
⚠️ **1つのショットは1つの `spec` を持つ**ので、**1つのショットが2つ以上の経路を持つことはできない**——同じショットを2つで撮って較べることは、まだできない。

⚠️ **このカードはノーマルフォーマットであり、3つの経路すべてがこれを埋める。**
時間の文法を自前では持たない——クリップの時間が何でできているかは経路が決め、
**経路がそれぞれそれを記述している**——`WAN 3.0` は連続した一本のテイクとして読み、
`MINIMAX H3` は独立したコマを自然なアニメーションで繋いだものとして組み、
`SEEDANCE 2.5` は長い一つのプロンプトとして読む——**そのプロンプトが自前の時計を
持てる**（`0-3s:` `3-6s:`）——そして**`Negative Prompt` の欄は、床としては
受け取られない**（`L30`）。⚠️ **この経路の制約のすべては
[`docs/seedance-route.md`](../docs/seedance-route-ja.md) に在る。**
⚠️ **ゆえに、§18 が何を言ってはならないかを決めるのは経路である。** `MINIMAX H3` では、
各コマがそれ自体で1つのシーンであり、次のコマとは自然なアニメーションでつながる——
⚠️ **だから §18 は、その継ぎ目を禁じてはならない。** **`no cut` を無条件の規則として
書くと、コマが場所を変える唯一の合法な手段が塞がり**、生成器は2つの場所を1つの部屋として
続けてしまう。**無関係な場所へのカットは禁じ、遷移そのものは禁じない**——そしてコマが
場所を変えるときは、**どちらなのかを仕様が書く。** 出典は `カット` という語を使わないので、
切るか繋ぐかはショットが決める。⚠️ **この経路に掛かる制約の一覧——渡す文字列と、
その紙の両方——は [`docs/h3-route.md`](../docs/h3-route-ja.md)。**
⚠️ **1つのショットは1つの `spec` を持つ**ので、**1つのショットが2つ以上の経路を持つことは
できない**——同じショットを2つで撮って較べることは、まだできない。
⚠️ **`L28` が読むのは仕様の §18 であり、`specmap.MODEL_ROUTE` と突き合わせる**——
このカードは開かない。⚠️ **ある経路の文法を別の経路の仕様へ持ち込むことが罠である**——
...
## 出典
Wan 3.0 を中間表現として書かれた動画生成の仕様であり、その後、残る2つの経路へ開かれた（[`docs/h3-route.md`](../docs/h3-route-ja.md)、[`docs/seedance-route.md`](../docs/seedance-route-ja.md)）——**このカードがどの経路から来たかは `HISTORY.md` に在る**；`storyboard`（最も近い静止画の先祖——コマとショット種別は持つが、移動・物理・音を持たない）
