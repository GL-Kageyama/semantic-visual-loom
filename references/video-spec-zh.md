...
登记在册的视频路径有两条——**`WAN 3.0` 附上 `key_image`**，
而**`MINIMAX H3` 附上分镜图像，根本没有图像路径**——
后者的纸由 `distill-essence-engine` 制作（格式 `storyboard`、样式 `luminous-anime`），
住在 `specs/board/<id>-board.md`。
⚠️ **一个镜头只持有一个 `spec`**，所以**一个镜头无法同时持有两条路径**——用两条路径拍同一个镜头再比较，现在还做不到。

⚠️ **两条路径不共享时间的文法，而本文开头那一行只写了其中一条。**
`Granularity×time: whole arc × one continuous take` 是 **`WAN 3.0` 的家风**，
**在 `MINIMAX H3` 上并不成立。** **在那里，每一格本身就是一场戏，
与下一格之间用自然的动画相接**——⚠️ **所以 §18 不得禁止这个接缝。**
**把 `no cut` 写成无条件的规则，会堵死一格改变地点的唯一合法手段**，
生成器就会把两个地点当成一个房间连下去。
**禁止切到无关的地点，而不禁止过渡**——并且当一格改变地点时，
**规格必须写明是哪一种。** 出处从不使用 `カット` 一词，所以切还是连，由那个镜头自己决定。
⚠️ **把开头那一行抄进 `H3` 规格的读者，抄的是另一条路径的规则**——
产生这条警告的失败记录在 `HISTORY.md` 里。
**读这一段的是 `L28`**（词语与理由在 `specmap.MODEL_ROUTE` 里）。
