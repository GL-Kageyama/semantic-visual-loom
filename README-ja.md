...
⚠️ **映像は、このリポジトリの中に在る。** `*.mp4`・`*.m4a`・`*.png` は **Git LFS** で入る
（[`.gitattributes`](.gitattributes)）——コミットされるのは**小さなテキストのポインタ**であり、
**絵そのものは GitHub 上の、このリポジトリの LFS ストレージに在る**——**別のリポジトリではない。**
⚠️ **`git-lfs` の無い clone は、映像ではなくポインタを降ろす**——clone の前に `git-lfs` を入れ、
あるいは後に `git lfs pull` を走らせる。
⚠️ **LFS はリポジトリとは別に計量され、それは所有者のアカウントに計上される**——
無料枠は**保存 10 GiB・転送 10 GiB/月**である
（[billing docs](https://docs.github.com/en/billing/concepts/product-billing/git-lfs)）。
**このリポジトリが実際に抱えている量は [`HISTORY.md`](HISTORY.md) に在る。**
