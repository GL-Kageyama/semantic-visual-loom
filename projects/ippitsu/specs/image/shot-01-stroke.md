...
- 生成器: `chatgpt-image-2.5`（種別 `image`）——`specmap.MODELS` の**唯一の画像の経路**である
  （著者の決定 2026-09-13「**画像の経路を担う。全ショットが通る。**」）
  ⚠️ **この行を読む検査は、まだ1つも無い**——だから何を書いても鳴らない。
...
- ⚠️ **この1枚は、このショットの動画へは渡さない。**（**著者の裁定、2026-09-22**）
  - ⚠️ **この行は、前は逆を書いていた**——「**この1枚は、このショットの動画へ添付（参照画像）として
    渡る**——最初のコマではない」。**著者がそれを否定した。**
  - **著者の言葉**: 「**参照イメージがない方が自然だったよ。参照があると、筆跡が画面上に、
    すでに乗ってしまう**」——**この1枚は `after` の側である**（墨は既に紙の上にあり、広がっている）。
    **このショットの主題は「墨が紙へ移る」という出来事そのもの**である——ゆえに
    **出来事の後の姿を参照として渡すことは、出来事と食い違う。**
  - ⚠️ **実測がこれを裏づけている**（`takes/ippitsu-s01-video-1.yaml` ／ `…-video-2.yaml`）。
    **参照を外した2本目が採用された。**
  - ⚠️ **この1枚は、それでもこの作品の基準画像である**——`ledger.yaml` の `紙.base` を見よ。
    **動画へ渡さないことは、要らないことではない。**
