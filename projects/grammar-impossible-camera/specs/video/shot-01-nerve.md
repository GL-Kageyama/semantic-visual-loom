...
**A single sheet of paper, and not a camera placed at a point** — **the frame is a page, so there is no position for it to move from.** The page holds for the whole shot and **the white is never crossed as a device.** **The style permits the frame to drift once, slowly; this shot spends none of it** — **the journey here is the wash crossing the white, and a drift would be a second motion in a frame whose only subject is that crossing.** No pan, no zoom, no rack, no travelling shot, **no conventional camera position, and no fisheye distortion standing in for an impossible viewpoint.** One continuous take; no cut.
