...
- 生成物の置き場: **`media/`**（作品の根から見た1箇所。`take.file` が名乗る先である）。
  ⚠️ **第二世代が在る**——`06_02_ChatGPT Image 2026年9月24日 00_58_18.png`（2026-09-24、焼き直し）。
  ⚠️ **第一世代は、このディレクトリに無い**（**実測**、2026-09-24——このディレクトリの `06_02_*` は、この1枚だけである）。
...
- `LOCATION`: the darkroom of the art board — the developing tray filling the lower part of the frame, one blank white silver-gelatin sheet lying flat under the surface of that liquid, its whole shape lying in the lower left of the tray and its near edge catching the amber light, with nothing floating up in it and no image on it, everything past the light's reach sunk in near-black
- `LIGHT`: the room's constant state — the dim red light of a safelight that stands outside the frame, and the warm amber glow of the developer tray; ⛔ **枠が持つのは光そのものであって、灯ではない**; the liquid takes the light rather than reflecting the room, so the film casts no shadow on the surface
...
- ⚠️ **焼かれた1枚**——`06_02_*`。**見えたままを書く。**
- ⛔ **芯は在る**——**手はフィルムを液へ下ろしており、指は年代と染みを持つ。**
- ⛔ **だが、3つ外れている。**
  **（1）トレイに沈んでいる白い紙が、見えない**——**この1本の `LOCATION` はそれを書いているのに、画に無い。**
  **（2）手に持たれているものが、小さなフィルム片でなく、幅の広い白い面に見える。**
  **（3）それは液に深く浸かっている**——**仕様は「端が触れ、液面が割れる」ところである。**
- ⚠️ **原因は、肯定の側の書きかたである**——**`LOCATION` の長い一文のなかに紙が埋もれ、
  フィルムは「one small strip」としか書かれていなかった**（**寸法が無い**）。
  ⛔ **ゆえに生成器は、その2つを1つに畳んだ。**
- ⛔ **直したのは5点である**——
  **（a）紙の位置を名指した**（液面の下・トレイの左下手前・近い縁が光を受ける・浮かない・動かない）／
  **（b）フィルムに寸法を与えた**（**幅は数センチ、紙のように薄い、指先で平らに挟む**）／
  **（c）2つを分ける一文を足した**（**同じ1つのものではない、と肯定の側で書く**）／
  **（d）`Negative` に「手に持つ大きな白い面」を足した**（**肯定の裏である**）／
  **（e）灯を枠の外へ出した**（**02・06-1 と同じ欠陥である**——動画の §16・§18 は禁じている）。
- ⚠️ **`no print in the tray` / `no photograph in the tray` は、この焼き直しでも残る**——
  **浮かぶのは、この1本ではフィルムの像である。**
