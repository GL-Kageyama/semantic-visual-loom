...
- `SUBJECT`: a woman's shadow lying thin on the darkroom wall, and, lower in the frame, the developer tray with its amber liquid and the red safelight above it — both of them unchanged
- `ACTION`: the shadow thickening very slowly, continuously, with no step in it, while the tray and the safelight do not change at all
- `LOCATION`: the darkroom of the art board — one windowless room, the wall taking the most of the frame, the tray below
- `LIGHT`: the room's constant state — one dim red safelight, which is also what lays the shadow on the wall, and the warm amber glow of the developer tray; no time of day and no directional light
