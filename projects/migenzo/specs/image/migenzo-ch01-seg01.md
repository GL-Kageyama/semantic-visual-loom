...
- 生成物の置き場: このディレクトリ。
  ...
  （**実測**、2026-09-24——このディレクトリの `01_01_*` は、**第三世代の1枚だけである**）。
  ⚠️ **その削除は `git status` に出ない**——**第二世代は追跡されていなかったからである。**
  ⚠️ **第三世代が在る**——`01_01_ChatGPT Image 2026年9月24日 02_28_30.png`（2026-09-24、著者が投入）。
  ⛔ **著者裁定（2026-09-24）——この1枚を採用する。** **選んだのは著者である**
  （`CLAUDE.md`「**生成はサンプルである。**」）。**ゆえに、この1本は採用済みである**
  （`PLAN.md` §4-f の第二巡）。⚠️ **残る外れは、下の第三世代の実測に置く**——**直さない**（裁定）。
...
## ⛔ 実測（2026-09-24、第三世代 n=1。**採用**）——灯は消え、紙は上がり、外れは3つ閉じた

- ⚠️ **焼かれた1枚**——`01_01_ChatGPT Image 2026年9月24日 02_28_30.png`。**見えたままを書く。**
- ⛔ **閉じた3点（実測）。**
  - **（1）灯**——**枠のどこにも器具は無い**（左上・上辺・右・トレイの外を拡大して確認。トレイの外は近黒、
    右上の画素は (4,3,3)）。**`LOCATION` の `near-black` も守られている。**
    赤は**光そのもの**として、左上の面に落ちているだけである。
  - **（3）均一な白い縁**——**無い。** 紙の右の縁は**まっすぐな裁ち切り**で、縁に沿うのは
    **数画素の明るい縁だけ**である（拡大して確認）。**像は紙の縁まで届いており、自分の縁を持たない。**
  - **（5）液から上がっている**——**満たされた。** 紙は液の上に吊られ、**下の縁から雫が二つ落ち**、
    下の液面は**静かで破れていない**。⚠️ **第一・第二世代の外れのうち、いちばん大きいものが閉じた。**
- ⛔ **（6）脂痕は、見える**——**眼の下・頬の上**に、**鈍い、縁の柔らかい濃いよごれ**。
  位置は仕様どおりである。⚠️ **大きさは眼の幅ほど**（**実測** 約77px 対 眼 約63px——親指の腹と同じ桁）。
  ⛔ **だが形は、下へ尾を引く滲み**であって、**指の腹の跡には見えない。**
  ⚠️ **そして仕様は「像の上の唯一のよごれ」と言うが、この1枚には細い明るい線（引っ掻き傷）が
  もう一本在る**——左の髪のあいだを縦に走っている（**実測**、拡大して確認）。
- ⛔ **（2）まだ肖像である。** ⚠️ **ただし第二世代の直りは保たれている**——**肌の上に、像の他の面と
  同じ粗い粒が乗っており**（**実測**、増感して拡大）、**口は閉じている**。
  ⛔ **残るのは、面の解像と光の宛がいである**——**像のなかで顔だけが解像し**（睫毛・眉・鼻・唇が読める）、
  **頬に整った光の勾配が乗っている**（`no portrait lighting` が在るのに、である）。
- ⛔ **（4）手は、まだ男性に読める**（**この1枚で、いちばん大きい残りである**）——
  **太い指・重い関節・立っている腱・短く四角い爪**。仕様の肯定は
  「**細い手首・長い指・静かに横たわる甲の腱**」を求めており、`Negative` は
  `no broad heavy hand` / `no sinewy or veined hand` / `no masculine hand` を持つ。
  ⚠️ **そしてこの手は、脂痕を置いた手である**——⚠️ **男性的に読めれば、`no second figure` の側からも
  部屋の人物が揺らぐ。**
- ⚠️ **（新）左に、彼女の体が入る**——黒い衣（上着とエプロン）と、その上の暖色の肌の面
  （**肩ないし首すじ**）。⛔ **眼・鼻・口は無い**ので **`no face in the room` は守られている** ✓。
  ⚠️ **ただし `CHARACTERS` は「手と前腕だけが枠に入る」と言っている。**
- ⚠️ **白について**——**この1枚の紙は、どこも暖かい焦げ茶で、中立な銀白の面は無い**
  （明るいところで R138 G78 B53、**実測**）。⛔ **ただし部屋の光そのものが赤である以上、
  画素では「銀白か」を決められない。** **観察として置く**——欠陥とは呼ばない。
- ⚠️ **`no small hand pressing the lens` は、この焼き直しでも掛かる**（**実測**: レンズも、
  レンズを押す手も、この1枚には無い）。
