...
- ⛔ **添付する参照画像（著者裁定、2026-09-23）**:
  ...
  ⚠️ **13本のうち、参照画像を持つのは2本である**——**この1枚（アートボード）と、05（絹の設定画）**
  （`PLAN.md` §4-c 問い11・問い14。**2026-09-23 著者裁定、同日 問い14 で05が加わった**）。
...
- 生成物の置き場: このディレクトリ。
  ⚠️ **第一世代が在った**——`12_01_ChatGPT Image 2026年9月23日 02_13_04.png`
  （2026-09-23、著者が投入。⚠️ **参照画像を添付した**——
  `生成時参照イラスト/12_01/` に、その1枚が残っている）。
  ⛔ **これは焼き直しと裁定された**（2026-09-24、著者）。⚠️ **そして、もう作業ディレクトリに無い**
  （**実測**、2026-09-24——`git status` が `D` を出す）。⛔ **この削除は、この稿では触っていない。**
  ⚠️ **第二世代が在る**——`12_01_ChatGPT Image 2026年9月24日 01_25_30.png`（2026-09-24、焼き直し）。
  ⛔ **著者裁定（2026-09-24）——この1枚を採用する。** **選んだのは著者である**（`CLAUDE.md`「**生成はサンプルである。**」）。

## ⚠️ 実測（2026-09-24、第二世代 n=1。**採用**）

- ⚠️ **焼かれた1枚**——`12_01_ChatGPT Image 2026年9月24日 01_25_30.png`。
- ⛔ **器具は、消えた。** **第一世代に在った「器具の頭と赤い口、腕」は、この1枚に無い**（左上を拡大して確認）。
- ⚠️ **`SUBJECT` のとおりである**——**何も写っていない銀白の紙が宙に吊られ、赤い帯がその上を横切り、
  トングが角を挟んでいる**（§9。**トングは枠の外から入る**）。**人は、この枠に居ない**（`CHARACTERS`）。
- ⛔ **そして、この判定は粗い。**——**この1枚について、私はこの3点しか測っていない。**
  **「光の島の縁が枠に保たれているか」（§10 の予算）は、測っていない。**
