...
- `SUBJECT`: a wet print lying flat in the amber developer, its full width in the frame, with an image in it — a back turned away and running, a small hand laid flat over the lens, and a white place where a face should be with no line between the white and the image — and a band of red light crossing that white
...
- `LIGHT`: the room's constant state — one dim red safelight that comes as far as the tray, and the warm amber glow of the developer; the band crossing the print is the safelight itself, and it is passing
